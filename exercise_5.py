...
def find_invalid_number(sequence, preamble_length):
    for i in range(preamble_length, len(sequence)):
        ...
        if not any(current_number == x + y for j, x in enumerate(preamble) for y in preamble[j + 1:]):
...
# Find first number that doesn't follow pattern
result = find_invalid_number(sequence, preamble_length)
print(f"The first number that cannot be expressed as a sum of two preceding numbers is: {result}")
...
def count_bags_inside(rules, bag_color):
    return sum(count * (1 + count_bags_inside(rules, color)) for count, color in rules.get(bag_color, []))


# Read bag rules from file
with open("data/input_bags.txt") as file:
    rules = {}
    for line in file:
        rule_parts = line.split()
        outer_bag_color = f"{rule_parts[0]} {rule_parts[1]}"
        inner_bags = [(int(rule_parts[i]), f"{rule_parts[i + 1]} {rule_parts[i + 2]}") for i in range(4, len(rule_parts), 4)]
        rules[outer_bag_color] = inner_bags



# Count the number of bags inside the target bag
total_bags_inside = count_bags_inside(rules)
print(f"The total number of bags inside the gold bag is: {total_bags_inside}")

# The first number that cannot be expressed as a sum of two preceding numbers is: 1639024365
#The total number of bags inside the gold bag is: 6260