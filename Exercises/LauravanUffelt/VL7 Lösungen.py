...
def open_file():
    input_path = Path(__file__).parents[2]/"data"/"breakout_commands.txt"
    with open(input_path, 'r') as file:
        commands = file.readlines()
        liste = []
        for line in commands:
            line = int(line.strip())  # convert into a int and removes whitespaces
            liste.append(line)  # appends evey command to a list
        liste[0] = 2  # changes the first command for task 2
        return liste
...
def analyse():
    ...
    global score
    global frame_data
    for index in range(0, len(frame_data), 3):  # goes through the commands (Schrittweite 3)
        x, y, type = frame_data[index], frame_data[index+1], frame_data[index+2] # saves the commands in variables
...
def input_setter():
    ...
    if ball_pos > paddle_pos:  # moves paddle
        return 1
...
def output_collector(data):
    frame_data.append(data)


values = open_file()
comp = int_computer.IntComputer(input_setter, output_collector)
comp.run(values)
