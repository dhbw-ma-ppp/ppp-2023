...
def main():
    elements = read_document()
    for element in elements:
        if element.startswith("$ ls") or element.startswith("dir"):
            continue
        if element.startswith("$ cd"):
            dest = element.split()[2]
            if dest == "..":
                stack.pop()
            else:
    ...
    replace = sum(n for n in sizes.values() if n <= 100000)
    return size, replace
