...
def get_arg_positions(index, mode, commands):
    positions = []
...
def get_output(commands):
    ...
    while commands[i] != 99:
        ...
        if opcode in opcodes.keys():
            positions = get_arg_positions(i, mode, commands)
            i = opcodes[opcode](i, positions, commands)
        else:
            raise KeyError("Wrong opcode")
...
eq_to_eight = [3,9,8,9,10,9,4,9,99,-1,8]
less_than_eigth = [3,3,1107,-1,8,3,4,3,99]
is_zero = [3,3,1105,-1,9,1101,0,0,12,4,12,99,1]
