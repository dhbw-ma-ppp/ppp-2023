...
liste1 = ["7", "g", "67", "fünf", "hh"]
liste2 = ["h", "j", "88", "5677777", "2", ["h"]]
liste3 = []
...
def listSort(arr):
  ...
  for i in range(0,len(arr)):
    ...
    try:
      ...
      if len(arr[i]) == 1:
        buchstaben.append(arr[i])
      else:
        print('weder zahl noch einzelner buchstabe an stelle ', i)
    except(TypeError) as exception:
      print('falscher eingabetyp an stelle ', i,)
    else:      
  ...
  if len(zahlen) > 0:
    print('zahlen: ', zahlen, '\n')
  if len(buchstaben) > 0:
    print('einzelne buchstaben: ', buchstaben)