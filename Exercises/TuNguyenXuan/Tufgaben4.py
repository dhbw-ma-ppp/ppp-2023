...
def cool_computing(arr):
    index = 0
    while True:
        opcode = int(str(arr[index])[-2:])
        if len(str(arr[index])) >= 3:
            if int(str(arr[index])[-2]) == 1:
                val_1 = int(arr[index+1])
            else:
                val_1 = int(arr[arr[index+1]])
        else:
            val_1 = int(arr[arr[index+1]])
        if len(str(arr[index])) >= 4:
            if int(str(arr[index])[-3]) == 1:
                val_2 = int(arr[index+2])
            else:
                val_2 = int(arr[arr[index+2]])
        else:
            val_2 = int(arr[arr[index+2]])
        match opcode:
            case 1:
                arr[arr[index+3]] = val_1 + val_2
                index += 4
            case 2:
                arr[arr[index+3]] = val_1 * val_2
                index += 4
            case 3:
                arr[arr[index+1]] = input("Type in your integer ")
                index += 2
            case 4:
