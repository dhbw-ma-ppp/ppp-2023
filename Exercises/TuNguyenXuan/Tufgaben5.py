...
def unsummable(seq):
    input = set(seq[:25])
    for index in range(25, len(seq)):
        target = seq[index]
        found = False
        for number in input:
            if target - number in input:
                found = True
                break
        if not found:
            return target
        input.remove(seq[index - 25])
        input.add(target)

print(unsummable(list1))
...
def count_of_bags(bag,bagdict):
    count = 0
    child = bagdict[bag].split(",")
    for element in child:
        numofelements = element[0]
        if numofelements != "n":
            count += int(numofelements)
            for childcount in range(int(numofelements)):
                count_of_bags(element[1:], bagdict)
    return count

print(count_of_bags("shinygoldbag",get_bag_dict("input_bags.txt")))
