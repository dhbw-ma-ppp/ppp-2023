...
test1 = ['1', 'd2', 'g', 'asdf', '-12', '+234', '4l;d9', 'h']
test2 = ['jalo', '456', 'g', '00', '-kell', '+=', '73812', 'h']
test3 = ['0', 'd', '3', 'a', '-1', '+', 'l', 'h']
...
def array_filter(arr):
    ...
    for i in range(0, len(arr)):
        if (len(arr[i]) == 1) and arr[i].isnumeric():
            integer_list.append(arr[i])
            char_list.append(arr[i])
        elif arr[i].isnumeric():
            integer_list.append(arr[i])
        elif len(arr[i]) == 1:
            char_list.append(arr[i])
    return integer_list, char_list


print(array_filter(test2))
