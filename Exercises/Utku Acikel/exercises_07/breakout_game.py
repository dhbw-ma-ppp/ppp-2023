...
filepath = Path(__file__).parents[3] / 'data' / 'breakout_commands.txt'
with open(filepath, 'r') as file:
    commands = [int(i) for i in file.read().split('\n') if i]
...
for index in range(0, len(commands), 3 ):
    if index + 2 < len(commands):
        x, y, tile_type = commands[index], commands[index+1], commands[index+2]
        game_state[(x, y)] = tile_type
        

# Creating a visual representation
screen = np.zeros((max(y for x, y in game_state.keys())+1, max(x for x, y in game_state.keys())+1))
for (x, y), tile_type in game_state.items():
