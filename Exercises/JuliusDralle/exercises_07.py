...
# Copyright to Atreju Tauschinsky
import operator
...
class BreakOut:
    ...
    def print_screen(self):
        for line_index in range(0,len(self.output)-2,3):
            if self.output[line_index] == -1 and self.output[line_index+1] == 0:
                score = self.output[line_index+2]
            else:
                x = self.output[line_index]
...
with open(path_dir_data / "breakout_commands.txt", "r") as input_file:
    input_lines = input_file.read().splitlines()
    input_lines_int = [int(x) for x in input_lines]
    input_lines = input_lines_int
    input_lines_int = None
...
computer = IntComputer(BRGame.input_function, BRGame.output_collect)

computer.run(input_lines)
