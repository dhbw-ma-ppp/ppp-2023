...
#commands = [1,2,3,0,2,2,2,1,3,3,99]
    ...
    while counter <= len(listCS):
        match listCS[counter]:
            ...
            case 4: #output the value at the position given
                print(f"commands[{counter}] = {listCS[counter]}")
                counter += 2
            ...
            case 6: #jump-if-flase
                if listCS[counter+1] == 0:
                    temp = counter
                    counter = listCS[temp+2]

            case 7: #less than
                if listCS[counter+1] < listCS[counter+2]:
            ...
            case _: #in case of an invalid Operator
                raise ValueError(f"Wrong opcode in line {counter}")
