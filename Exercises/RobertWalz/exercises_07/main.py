...
exercises_dir = here.parent.parent
data_dir = exercises_dir.parent / "data"


# get the commands
with open(data_dir / "breakout_commands.txt", "r") as file:
    memory = {key: int(line.rstrip("\n")) for key, line in enumerate(file.readlines())}
...
class Game:
    ...
    def score_display(self, screen):
        ...
        effects = [
            Cycle(
                screen, FigletText("Breakout", font="big"), int(screen.height / 2 - 8)
            ),
            Cycle(
                screen, FigletText(score_str, font="big"), int(screen.height / 2 + 3)
            ),
            Stars(screen, 200),
        ]
    ...
    def render(self, screen: Screen):
        ...
        for key, item in self.pixel_dict.items():
            ...
            screen.print_at(
                item[0], int(xy_pos[0]), int(xy_pos[1]), item[1]
            )  # print the pixel
...
def task2():
    ...
    computer = Computer(memory=memory, input_getter=game.input_getter, output_stream=game.split_triplets)
    response = computer.run()
    Screen.wrapper(game.score_display)
    print(f"Computer finished execution with Code: {response}")
