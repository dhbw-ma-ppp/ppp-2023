...
def execute_program(commands, input_value):
    ...
    while i < len(commands):
        instruction = str(commands[i]).zfill(5)
        ...
        if opcode == 1:
            commands[commands[i+3]] = get_value(i+1, param_modes[0]) + get_value(i+2, param_modes[1])
            i += 4
        elif opcode == 2:
            commands[commands[i+3]] = get_value(i+1, param_modes[0]) * get_value(i+2, param_modes[1])
            i += 4
        elif opcode == 3:
            commands[commands[i+1]] = input_value
            i += 2
        elif opcode == 4:
            output = get_value(i+1, param_modes[0])
            i += 2
        elif opcode == 5:
            if get_value(i+1, param_modes[0]) != 0:
                i = get_value(i+2, param_modes[1])
            else:
                i += 3
        elif opcode == 6:
            if get_value(i+1, param_modes[0]) == 0:
            ...
            commands[commands[i+3]] = 1 if get_value(i+1, param_modes[0]) < get_value(i+2, param_modes[1]) else 0
            i += 4
        elif opcode == 8:
            commands[commands[i+3]] = 1 if get_value(i+1, param_modes[0]) == get_value(i+2, param_modes[1]) else 0
            i += 4
